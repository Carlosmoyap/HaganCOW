...
function process_reservation_request($post, $server)
{
    ...
    $hotelNom = isset($post['hotelNom']) ? neteja_dades($post['hotelNom']) : '';
    if ($hotelNom !== '' && mb_strlen($hotelNom) > 150) {
        $result['errors'][] = 'El nom de l\'hotel no pot superar els 150 caracters';
    }
    $result['dadesReserva']['hotelNom'] = ($hotelNom !== '') ? $hotelNom : 'Sense hotel especificat';
...
{
    return open_db_connection($dbWarning);
}
...
function ensure_reservation_table($conn)
{
    ...
    if (!(bool)$conn->query($sqlReserves)) {
        return false;
    }

    $hasHotelColumn = false;
    $colResult = $conn->query("SHOW COLUMNS FROM clients_reserves LIKE 'hotel_name'");
    if ($colResult) {
        $hasHotelColumn = $colResult->num_rows > 0;
        $colResult->free();
    }

    if (!$hasHotelColumn) {
        if (!(bool)$conn->query("ALTER TABLE clients_reserves ADD COLUMN hotel_name VARCHAR(150) NULL AFTER city_name")) {
            return false;
        }
    }
...
function save_reservation($conn, &$result, $codiReserva, $preuTotal)
{
    $insertSql = 'INSERT INTO clients_reserves
        (reservation_code, client_name, email, phone, city_id, city_name, hotel_name, checkin_date, checkout_date, nights, guests, room_type, comments, total_price)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
{
    $recentReservations = array();
    $recentSql = 'SELECT reservation_code, client_name, city_name, hotel_name, checkin_date, checkout_date, total_price
